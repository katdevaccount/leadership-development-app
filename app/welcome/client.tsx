...
export function WelcomeClient({ copy }: WelcomeClientProps) {
  ...
  const getPlaceholder = (index: number): string => {
    if (copy.ideasPlaceholders && copy.ideasPlaceholders[index]) {
      return `e.g., ${copy.ideasPlaceholders[index]}`
    }
    return 'Add an idea or experiment...'
  }
